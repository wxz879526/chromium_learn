...
int CMFCApplicationApp::Run()
{
	...
	logSettings.log_file_path = L"test.log";
	bool b = logging::InitLogging(logSettings);
	if (b)
	{
		logging::SetLogItems(true, true, true, false);
	}

	LOG(INFO) << "CMFCDemoApp run";

	base::MessageLoop mainLoop(base::MessagePumpType::UI);
	base::ThreadTaskRunnerHandle::Get()->PostTask(FROM_HERE, base::BindOnce([]() {
		OutputDebugStringA("12323444");
	}));
